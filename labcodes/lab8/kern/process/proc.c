...
alloc_proc(void) {
     ...
     */
     //LAB5 2013011365 : (update LAB4 steps)
    /*
...
do_fork(uint32_t clone_flags, uintptr_t stack, struct trapframe *tf) {
    ...
    if ((ret = copy_files(clone_flags, proc)) != 0)		// LAB 8
...
load_icode(int fd, int argc, char **kargv) {
    ...
    uint32_t sz = 0;
    for(i = 0; i < argc; ++i) {
    	sz += strnlen(kargv[i], EXEC_MAX_ARG_LEN + 1) + 1;
    }

    uintptr_t top = USTACKTOP - (sz / sizeof(long) + 1) * sizeof(long);
    char** argv = (char**)(top - argc * sizeof(char*));

    sz = 0;
    for(i = 0; i < argc; ++i) {
    	argv[i] = strcpy((char*)(top+sz), kargv[i]);
    	sz += strnlen(kargv[i], EXEC_MAX_ARG_LEN + 1) + 1;
    }

	top = (uintptr_t)argv - sizeof(int);
    *(int*)top = argc;

    //(6) setup trapframe for user environment
    struct trapframe *tf = current->tf;
    memset(tf, 0, sizeof(struct trapframe));
    /* LAB5:EXERCISE1 2013011365
