...
do_pgfault(struct mm_struct *mm, uint32_t error_code, uintptr_t addr) {
    ...
    */
    	if(swap_init_ok) {
            ...
            int rr = page_insert(mm->pgdir, page, addr, perm); //(2) According to the mm, addr AND page, setup the map of phy addr <---> logical addr
            if (rr != 0) {
            	cprintf("Insert Page to mm->pgdir Failed!\n");
            	goto failed;
            }
            rr = swap_map_swappable(mm, addr, page, 0); //(3) make the page swappable.
            if (rr != 0) {
